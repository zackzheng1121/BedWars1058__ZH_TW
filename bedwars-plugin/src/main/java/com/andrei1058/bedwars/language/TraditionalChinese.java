...
public class SimplifiedChinese extends Language {
    ...
    public SimplifiedChinese() {
        ...
        yml.addDefault(Messages.COMMAND_MAIN, Arrays.asList("", "&2▪ &7/" + mainCmd + " stats", "&2▪ &7/" + mainCmd + " join &o<遊戲/模式>", "&2▪ &7/" + mainCmd + " leave", "&2▪ &7/" + mainCmd + " lang", "&2▪ &7/" + mainCmd + " gui", "&2▪ &7/" + mainCmd + " start &3（贊助者）"));
        yml.addDefault(Messages.COMMAND_LANG_LIST_HEADER, "{prefix} &2可以使用的語言：");
        ...
        yml.addDefault(Messages.COMMAND_LANG_SELECTED_NOT_EXIST, "{prefix}&c該語言不存在！");
        yml.addDefault(Messages.COMMAND_LANG_SELECTED_SUCCESSFULLY, "{prefix}&a已成功設置該語言！");
        yml.addDefault(Messages.COMMAND_LANG_USAGE_DENIED, "{prefix}&c你不能在遊戲進行時變更語言。");
        yml.addDefault(Messages.COMMAND_JOIN_USAGE, "&a▪ &7用法：/" + BedWars.mainCmd + " join &o<遊戲/模式>");
        yml.addDefault(Messages.COMMAND_JOIN_GROUP_OR_ARENA_NOT_FOUND, "{prefix}&c遊戲{name}不存在！");
        yml.addDefault(Messages.COMMAND_JOIN_DENIED_IS_FULL, "{prefix}&c遊戲已滿！\n&a請考慮贊助我們！ &7&o(點擊查看)");
